...
function MetricCard({ label, value, unit, icon: Icon, accent }: {
  ...
  return (
    <div className={`rounded-xl p-4 card-elevated ${accent ? 'bg-gradient-green text-primary-foreground' : 'bg-card'}`}>
      <div className={`flex items-center gap-2 mb-1 ${accent ? 'text-primary-foreground/70' : 'text-muted-foreground'}`}>
        <Icon className="h-4 w-4" />
        <span className="text-xs font-semibold uppercase tracking-wider">{label}</span>
      </div>
      <div className="flex items-baseline gap-1.5">
        <span className={`text-2xl font-bold font-mono animate-count-up ${accent ? 'text-primary-foreground' : 'text-foreground'}`}>
          {value}
        </span>
        <span className={`text-sm ${accent ? 'text-primary-foreground/60' : 'text-muted-foreground'}`}>{unit}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-sm font-semibold uppercase tracking-widest text-muted-foreground">Total Lifecycle Carbon Score</h2>
